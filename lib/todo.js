const duplicate = function ({ id, name, tasks }) {
  return { id, name, tasks: tasks.map(task => Object.assign({}, task)) };
};
...
class TodoRecords {
  ...
  addTodoTask(todoId, taskName) {
    ...
    return !todo ? false : todo.addTask(taskName);
  }
  ...
  editTaskName(todoId, taskId, name) {
    const todo = this.findTodo(todoId);
    return !todo ? false : todo.editTask(taskId, name);
  }

  changeTaskStatus(todoId, taskId) {
    const todo = this.findTodo(todoId);
    return !todo ? false : todo.changeStatus(taskId);
  }

  deleteTodoTask(todoId, taskId) {
    const todo = this.findTodo(todoId);
    return !todo ? false : todo.deleteTask(taskId);
  }
