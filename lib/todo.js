...
class Todo {
  ...
  generateId() {
    const task = this.tasks[this.tasks.length - 1] || 0;
    return task.id + 1;
  }
...
class TodoRecords {
  ...
  addTodo(todo) {
    // const newTodo = new Todo(todo.name, todo.tasks, getTodoListId());
    todo.id = this.generateId();
    this.todoLists.push(Todo.create(todo));
  }

  generateId() {
    const todo = this.todoLists[this.todoLists.length - 1] || 1;
    return todo.id + 1;
  }
