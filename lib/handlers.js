...
const { TodoRecords, Tasks, Todo } = require('./todo');
...
const TODO = TodoRecords.loadTodo(JSON.parse(loadTodos()).todoLists);
// console.log(TODO);
...
const addNewList = function(req, res) {
  ...
  newList = new Todo(newList.name, newList.tasks);
  TODO.addTodo(newList);
  console.log(TODO);
  // TODOS.push(JSON.parse(req.body));
  fs.writeFileSync(LIST_STORE, JSON.stringify(TODO), 'utf8');
